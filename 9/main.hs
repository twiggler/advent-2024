...
data Block = FileId Int | Empty deriving (Show, Eq)
...
blocks = zip [0 ..] >=> expand
  where
    expand (i, n) = replicate n (if even i then FileId (i `div` 2) else Empty)
...
compactDisk = go . Seq.fromList
  where
    ...
    go ((Empty :<| rest) :|> FileId t) = t :<| go rest -- Defrag, move block to the left
...
defragDisk (fileRegions, freeRegions) =
  ...
  where
    ...
    moveToLeft r@(FileRegion fid (Region sourceOffset sourceSize)) = do
      emptyRegions <- St.get
