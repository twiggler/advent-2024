...
readCodes :: FilePath -> IO [String]
readCodes filename = lines <$> readFile filename
...
expand levels = paths numericKeypad >=> foldMap (traverse (go levels) >=> shortestChoice)
  where
    ...
    paths keypad code' = traverse (directionsForHop keypad) (buttonHops keypad code')
    ...
    -- After the initial mapping from the numeric keypad at the highest level, the remaining
    -- entries map the set of paths S to its cartesian product S x S
    directions =
